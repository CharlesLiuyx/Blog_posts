...
这时候时间复杂度优化为 $O(mhlog_2(h))$。此时，如果答应一下所有状态转移的过程（行为鸡蛋数 $m$，从2开始；列为楼高 $h$ ，从1开始），如下图（红色框表示 $k$ 的移动，蓝色双向箭头表示取大的一个，然后还需要加1后和之前的 $f(m,h)$ 比选小的一个。下图展示的是 $m_i=2,h_j=11,k=5$ 时的情况）
