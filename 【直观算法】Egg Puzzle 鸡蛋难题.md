...
在一层楼，丢了一个鸡蛋，除了`碎了`这个结果，还有另一种可能性，即`没碎`。这句话是一个**极为关键的信号**，背后在告诉我们这是一个**树形结构**问题
...
假设第一个鸡蛋的楼层策略是 $k_1, k_2,\cdots,k_p$ ，其中 $p$ 是`仍的总次数`，楼高记为 $N$ 。比如如果你选择在 $20,40,60,80$ 层仍，那么 $p=4$ ，根据题意，有 $1 \lt k_1 \lt k_2 \lt \cdots \lt k_{p-1} \lt k_p =N$ 。画出树形示意图，如下图所示

<div align="center"><img src="【直观算法】Egg Puzzle 鸡蛋难题/Tree.png" alt="" width="700"></div>

图最上方的数轴就是一个**横放的楼**，$k_0 = 1$ 是第一层

解释一下这棵树。在每一层扔下一个鸡蛋时，都有两种可能，`碎了`和`没碎`，分别对应了两个分支。一旦`碎了`，那么就使用`1`个鸡蛋的问题思路来解题，直到找到**临界层**。在 $k\_1$层没碎，那么需要在 $k\_2$ 层继续尝试，直到最高层 $N$ 找到**临界层**（上图的标注不是树高，是灰色图例标识出的的**子树高**）
